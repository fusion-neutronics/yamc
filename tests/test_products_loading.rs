        ...
        for (i, product) in reaction.products.iter().enumerate() {
            ...
            if !product.distribution.is_empty() {
                match &product.distribution[0] {
                    materials_for_mc::reaction_product::AngleEnergyDistribution::UncorrelatedAngleEnergy { angle, energy } => {
                        println!("  Angle distribution has {} energy points", angle.energy.len());
                        match energy {
                            Some(materials_for_mc::reaction_product::EnergyDistribution::LevelInelastic { .. }) => {
