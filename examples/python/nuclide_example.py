...
for i, (mt, reaction) in enumerate(reactions_with_products[:3]):
    print(f"\n--- Reaction MT {mt} ---")
    ...
    for j, dist in enumerate(product['distribution']):
        print(f"  Distribution {j+1}: {dist['type']}")
        
        if dist['type'] == 'UncorrelatedAngleEnergy':
            ...
            print(f"      First mu distribution: x = {first_mu['x']}, p = {first_mu['p']}")
            
            # Check if energy distribution is present
            if 'energy' in dist:
                energy_dist = dist['energy']
                print(f"    Energy distribution: {energy_dist['type']}")
                
        elif dist['type'] == 'KalbachMann':
            ...
            print(f"      Energy_out data available: {'energy_out' in dist}")
            print(f"      Slope data available: {'slope' in dist}")
